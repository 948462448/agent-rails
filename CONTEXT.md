...
- Every Compatibility Shell invokes the Agent Rails Python CLI through `python -E` and the Trusted Python Bootstrap; `PYTHONPATH` plus `python -m` from a Target Project is not an allowed runtime seam. Internal absolute Python helpers use the same environment isolation.
