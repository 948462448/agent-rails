...
快照检索出的少量相关源码和测试位置。同一固定 target 的相同失败连续出现时，第二次
...
任务或评分标准已经冻结时，可以把原文显式交给 Task Pack，避免目标摘要丢失验收细节：

```bash
agent-rails pack --task-file task.md --rubric-file rubric.md "按冻结合同实现"
```

完整合同不会被 token 预算静默截断；预算不足或目标提到附件却未传文件时，命令会直接失败。
