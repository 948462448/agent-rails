...
Local Brain 适合训练的是“有界决策”，不是完整 coding 能力。学习单元必须继续受现有
Interface 约束：从枚举候选中选择 capability、排序 Code Evidence、判断是否回退，不能
在训练后扩大为命令生成、文件写入或验证裁决。

### 12.1 Local Brain Learning Loop Module

Local Brain Learning Loop 是一个离线、版本化 Module，其 Interface 接受脱敏决策 trace、
确定性基线和任务结果，产出可审计的数据集与候选模型报告。它不属于 Brain runtime，
也不允许运行中的 Adapter 修改或替换自身权重。

每条可训练 trace 至少记录：

- request ID、decision schema、模型/Adapter 版本和 Capability Catalog hash；
- 确定性候选、Brain 决策、validator 结果、采用/回退状态与原因；
- 主 agent 是否采用 capability 建议，以及后续确定性验证结果；
- Task Pack/session token、读取数、工具调用数、修复轮数和延迟；
- 数据策略版本和脱敏结果。

默认数据集只保留结构化候选 ID、角色、规范化原因、结果和成本指标，不保留凭证、remote
URL、Profile 环境、完整源码或原始第三方 capability 描述。确实需要局部代码训练时，
必须显式启用私有数据集、再次脱敏并经过人工抽检；任何上传或外部 GPU 训练都需要单独
授权。删除这个 Module 会让 trace schema、数据脱敏、奖励计算、模型评测和晋升规则散落
到 Brain Host、eval 脚本与 Adapter，因此它通过删除测试，能为训练工具提供更高 Depth，
并把数据治理与晋升错误集中在一个具有 Locality 的实现中。

### 12.2 分阶段训练

1. **Trace 基线**：先运行 `shadow`，收集 deterministic/Brain 差异和真实结果，不训练；
2. **SFT/LoRA**：学习严格 JSON、合法 ID、Capability Catalog 词汇、角色约束和基础排序；
3. **DPO**：从 deterministic/Brain、采用/拒绝和成功/失败的配对决策中学习偏好；
4. **GRPO/RLVR**：奖励与离线评测稳定后，才优化可以由 Rails 验证的结果；
5. **部署**：合并或转换候选权重，量化为 MLX 运行产物，先进入 `shadow`；
6. **晋升**：离线 eval、`shadow` 和小范围 `assist` 均无安全/质量回退后，才显式升级。

MLX 4-bit 模型是部署产物，不等于训练格式。Mac 上可以用 MLX-LM 做 LoRA/QLoRA SFT；
DPO、GRPO 等训练可在支持的 GPU/Linux 训练栈完成，再转换和量化回本地 MLX。训练框架是
Local Brain Learning Loop 的 Adapter，不进入 Brain runtime 的稳定 Interface。

### 12.3 奖励与防投机

奖励按优先级分层，硬约束不能用成本收益抵消：

1. **安全与协议**：JSON/schema 合法，只选择现有 available ID，不破坏 required roles，
   无越权字段；违反任一项直接判负并回退；
2. **任务结果**：Rails 的确定性验证和发布门禁结果、首次修改通过率、修复轮数；
3. **决策效用**：证据是否被读取、capability 建议是否被采用、是否减少重复读取；
4. **成本**：完整 session token、工具调用、本地推理延迟与资源占用。

不能只奖励“测试通过”或“调用更少”，否则模型可能选择更弱的测试、过早回退或遗漏必要
证据。训练和晋升必须使用固定验证覆盖、保留集和对抗样本；删除/弱化测试、绕过验证、
降低 required role 覆盖或提高安全回退率都记为显式惩罚。奖励定义、数据集、模型、prompt
和 Catalog 版本必须一起固化，禁止在同一评测中移动目标。

### 12.4 晋升门禁

候选模型只有同时满足以下条件才可从离线进入 `shadow`，再进入 `assist`：

- held-out paired eval 的任务成功率不低于当前模型和 deterministic 基线；
- 协议、安全、敏感信息和 required-role 指标无回退；
- `shadow` 的超时、无效响应、回退率和资源成本在预算内；
- `assist` 在成功率不降低时，至少稳定改善总 token、读取数或修复轮数之一；
- 晋升由人显式更新版本，保留旧模型与 `off` 回滚路径。

参考实现能力以官方资料为准：[MiniCPM](https://github.com/OpenBMB/MiniCPM) 记录了
MiniCPM5 的 SFT/RL 路线与常见微调框架；[MLX-LM LoRA](https://github.com/ml-explore/mlx-lm/blob/main/mlx_lm/LORA.md)
支持 Apple Silicon 上的 LoRA/QLoRA；[TRL](https://huggingface.co/docs/trl/quickstart)
提供 SFT、DPO、GRPO trainer；[verl](https://github.com/verl-project/verl) 可作为 GPU
强化学习训练 Adapter 的候选。引用这些工具不代表把它们加入 Agent Rails 基础依赖。
...
- 固化脱敏 trace schema、数据保留和显式导出策略；
- 先生成 SFT/DPO 数据集和离线报告，不自动启动训练；
- 奖励稳定后增加 GRPO/RLVR 实验 Adapter；
- 量化候选模型必须经过离线、`shadow`、`assist` 三道门禁；
- 模型晋升和回滚始终是显式操作，runtime 不自训练、不自动替换权重。
...
- 不让 runtime 使用在线反馈自训练、自改 prompt 或自动替换模型；
- 不把未脱敏 trace、源码或凭证自动上传到外部训练服务；
...
9. eval 延后到运行功能稳定后完善，但从第一刀开始保留 request ID、回退原因和可选私有
   trace，避免以后无法测量。
10. 持续学习按 trace → SFT/LoRA → DPO → GRPO/RLVR → MLX 量化 → shadow/assist 门禁
    演进；训练与部署分离，runtime 永不自训练或自动晋升。
