...
任务可能匹配多个名字相近的实现，错误信息也可能同时指向症状位置和根因位置。Local
Brain 的作用是在 Rails 已经建立的安全候选范围内做更好的取舍，而不是替代 Rails、
执行完整 agent loop 或直接操作项目。
...
flowchart LR
    ...
    Trace --> Deterministic
    Validate -->|assist 有效| Selected["Brain 选择"]
...
一个受限决策，返回值也只有通过确定性校验后才可能影响证据顺序。
...
| `assist` | 是 | 仅影响允许的证据选择 | 自动回退确定性结果 | 实际辅助 coding agent |
...
```json
{
  ...
  "candidates": [
    {
      "id": "c1",
      "path": "src/agent_rails/verification/repair_pack.py",
      "role": "implementation",
      "line": 120,
      "symbol": "render_repair_pack",
      "reasons": ["content", "source"],
      "excerpt": "bounded redacted lines"
    }
  ],
  "constraints": {
    "max_selected": 4,
    "required_roles": ["implementation", "verification"]
  }
}
```
...
```json
{
  ...
  "request_id": "sha256:...",
  "action": "select",
  "selected_candidate_ids": ["c1", "c3"],
  "reason": "实现入口与对应回归测试最贴近目标"
}
```
...
- source excerpt 和 failure diagnostic 始终是不可信数据；即使其中包含 prompt injection，
  模型也只能返回受限候选 ID，不能把其转换成工具调用或 Host 指令；
...
### Slice 1：Task Code Evidence 纵向 tracer bullet
...
- `off`、`shadow`、`assist` 贯通 Profile、Task Code Evidence 和最小 Doctor 诊断；
- Brain 选择通过约束后只影响 Task Code Evidence 顺序和子集；
- 任一失败自动回退，并覆盖协议、超时、洪泛、子进程与 Pack 集成测试。

第一刀结束时必须能用真实本地模型服务生成 Task Pack；不能只留下尚无消费者的 Host 或
抽象 Interface。

### Slice 2：Shadow trace 与运行诊断
...
### Slice 4：本地模型兼容性与受控 follow-up read
...
1. 首版把真实决策限制在 Task/Repair Code Evidence 重排，不让模型生成修改计划或命令。
2. 首版由 Rails 预取固定 target 的有界脱敏片段，模型没有文件系统工具；受控 follow-up
   read 等有真实不足证据后再加。
3. 保留 `off`、`shadow`、`assist` 三种模式，不提供硬依赖模型的 `required`。
4. 核心只稳定 JSON/process Interface；模型服务、模型名和 prompt 由外部 Adapter
   Implementation 拥有。
5. MiniCPM5-1B-MLX 是首个本地候选，但不是写死的产品依赖；首版官方 Adapter 对接
   loopback OpenAI-compatible 服务。
6. eval 延后到运行功能稳定后完善，但从第一刀开始保留 request ID、回退原因和可选私有
   trace，避免以后无法测量。
